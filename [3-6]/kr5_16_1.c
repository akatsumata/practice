...
int main(int argc, char *argv[])
{
    	...
    	if((nlines = readlines(lineptr, MAXLINES)) >= 0){
        	l_qsort((void **) lineptr, 0, nlines - 1,
            		(int (*)(void *, void*))(numeric ? l_numcmp : l_strcmp));
...
int l_strcmp(char *s, char *t)
{
	...
	if(fold == 1){
           	while(*s == *t){
                  	for(; !isalnum(*s) && *s != ' ' && *s != '\0'; s++){
                                ;
                        }
                        for(; !isalnum(*t) && *t != ' ' && *t != '\0'; t++){
                                ;
                        }
                        if(tolower(*s) == tolower(*t) && *s == '\0'){
                                return 0;
                        }
                        *s++;
                        *t++;
